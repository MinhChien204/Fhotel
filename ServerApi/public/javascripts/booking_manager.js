...
async function fetchBookings() {
  try {
    ...
    bookings.forEach((booking, index) => {
      const isDisabled = booking.status === "confirmed" || booking.status === "cancelled";
...
async function updateBookingStatus(bookingId, newStatus) {
  try {
    ...
    if (response.ok) {
      ...
      const dropdown = document.querySelector(`.status-dropdown[data-booking-id="${bookingId}"]`);
      if (dropdown) {
        dropdown.value = newStatus;

        // Nếu trạng thái là "confirmed" hoặc "cancelled", disable dropdown
